...
void nose_init(){
    ...
    else if(level == 2)
    {
        ...
        for( int i=0;i<16;i++)
        {
            int min = 0, max = 7;
            nose[i].type=rand() % (max - min + 1) + min;
            if(nose[i].type==7||nose[i].type==6)nose[i].img_move = al_load_bitmap("./image/coin.png");
            else if(nose[i].type==5)nose[i].img_move = al_load_bitmap("./image/star.png");
            else if(nose[i].type==4)nose[i].img_move = al_load_bitmap("./image/bubble.png");
